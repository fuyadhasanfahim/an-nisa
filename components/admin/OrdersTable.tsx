...
function sortTupleFromSelect(
  v: string
): { sort: OrderListSortField; order: "asc" | "desc" } {
  const idx = v.indexOf(":");
...
function OrderActionsCell({ orderId }: { orderId: string }) {
...
function buildColumns(col: ReturnType<typeof createColumnHelper<Row>>) {
  return [
    ...
    col.accessor("customerEmail", {
      ...
      cell: (ctx) => (
        <div className="min-w-0">
          ...
          <div className="truncate text-xs text-black/55">{ctx.getValue()}</div>
        </div>
      ),
    }),
    col.display({
    ...
    col.display({
      ...
      cell: (ctx) => <OrderActionsCell orderId={ctx.row.original.id} />,
    }),
  ];
}
  ...
  if (data && data.total === 0 && !listQuery.q) {
    return (
      <div className="rounded-xl bg-white p-8 shadow-sm ring-1 ring-black/5">
        <div className="text-sm font-medium text-brand-black">No orders yet</div>
    ...
    <div className="space-y-4">
      ...
      {data && data.total === 0 && listQuery.q ? (
        <div className="rounded-xl bg-white p-8 text-center text-sm text-black/60 shadow-sm ring-1 ring-black/5">
          No orders match &ldquo;{listQuery.q}&rdquo;. Try a different search.
        </div>
      ) : (
        <div
          className={[
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <button
              ...
              className={[
                ...
                "ring-1 ring-black/10 transition hover:bg-black/5",
                "disabled:cursor-not-allowed disabled:opacity-45",
                "focus:outline-none focus:outline-none focus-visible:ring-2 focus-visible:ring-brand-pink/40",
              ].join(" ")}
            >
              Next
