...
export async function PATCH(
    ...
    try {
        ...
        if (Array.isArray(body.items)) {
            for (const it of body.items) {
                if (
                    ...
                    await prisma.orderItem.updateMany({
                        where: { orderId: id, productId: it.productId },
                        data: { quantity: Math.round(it.quantity) },
                    });
                }
            }
            ...
            const newSubtotalCents = updatedItems.reduce(
                (acc, item) => acc + item.unitCents * item.quantity,
                0,
            );
            const newTotalCents =
                newSubtotalCents + order.shippingFeeCents - order.discountCents;

            allowedUpdates.subtotalCents = newSubtotalCents;
            allowedUpdates.totalCents = newTotalCents;
        }
